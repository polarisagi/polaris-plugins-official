...
def handle_open_app(args):
    ...
    if open_name and open_name.lower() != "desktop":
        ...
        if plat == "Darwin":
            ...
            r_check = subprocess.run(["osascript", "-e", f'id of application "{open_name}"'], capture_output=True)
            ...
            if shutil.which(open_name):
                ...
                ps_cmd = f"Get-StartApps | Where-Object {{$_.Name -match '{open_name}'}}"
                r_check = subprocess.run(["powershell", "-command", ps_cmd], capture_output=True, text=True)
            ...
            if shutil.which(open_name):
                ...
                r_check = subprocess.run(f"ls /usr/share/applications/*{open_name}*.desktop ~/.local/share/applications/*{open_name}*.desktop", shell=True, capture_output=True)
                exists = (r_check.returncode == 0)
                
        if not exists:
            return [{"type": "text", "text": f"Error: Application '{open_name}' is not installed or not found. Please try a different name or ask the user."}]
    ...
    if plat == "Darwin":
        ...
        if not success and open_name.lower() != "desktop":
            ...
            try:
                subprocess.run(["open", "-a", open_name], check=True, capture_output=True)
        ...
        if open_name.lower() == "desktop":
            ...
            subprocess.Popen(f'start "" "{open_name}"', shell=True)
...
def handle_get_running_apps(args):
    ...
    if plat == "Darwin":
        ...
        try:
            out = subprocess.check_output(["powershell", "-command", "Get-Process | Where-Object {$_.MainWindowTitle -ne ''} | Select-Object -ExpandProperty Name"], timeout=5).decode().strip()
            apps = list(set([name.strip() for name in out.splitlines() if name.strip()]))
        except Exception as e:
            return [{"type": "text", "text": f"Error getting apps: {e}"}]
    else:
        try:
            out = subprocess.check_output(["wmctrl", "-l"], timeout=5).decode().strip()
            apps = [line.split(None, 3)[-1] for line in out.splitlines() if len(line.split()) >= 4]
        except Exception as e:
            return [{"type": "text", "text": f"Error getting apps (requires wmctrl): {e}"}]
            
    return [{"type": "text", "text": "Running applications:\n" + "\n".join(f"- {app}" for app in apps)}]
...
def handle_send_message_to(args):
    ...
    if not contact_name: raise Exception("send_message_to requires 'contact_name'")
    if not message: raise Exception("send_message_to requires 'message'")
