...
def handle_get_screen_state(args):
    ...
    elements plus OCR-detected text — with unique IDs instead of coordinates.
    """
    ...
    try:
        ...
        if ax_result and ax_result[0].get("text"):
    ...
    if platform.system() == "Darwin" and _ensure_describe_binary():
        tmp_path = ""
        try:
            tmp_path, x_off, y_off = utils.capture_screen(app_name=app_name)

            if tmp_path and os.path.exists(tmp_path):
                ocr_raw = subprocess.check_output(
                    [_DESCRIBE_BIN, tmp_path], timeout=15
                ).decode().strip()
                if ocr_raw:
                    lines.append("\n=== Screen Text (OCR) ===")
                    for line in ocr_raw.splitlines()[:100]:
                        parts = line.split("|")
                        if len(parts) == 3:
                            text = parts[0]
...
def handle_screenshot(args):
    ...
    try:
        ...
        state_data = {"metadata": {"app_name": app_name, "timestamp": time.time()}, "elements": {}}
        with open(state_file, "w") as f:
            json.dump(state_data, f)
    except Exception:
        pass
